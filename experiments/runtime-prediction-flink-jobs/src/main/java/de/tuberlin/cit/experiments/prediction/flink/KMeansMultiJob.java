...
public class KMeansMultiJob {
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i = 1; i <= numIterations; i++) {
			...
			centroids.write(new TypeSerializerOutputFormat<>(),
					(intermediateResultsPath + "/iteration_" + Integer.toString(i)),
					FileSystem.WriteMode.OVERWRITE).setParallelism(1);

			env.execute("KMeans - Iteration " + Integer.toString(i));
		}
	...
	private static boolean parseParameters(String[] programArguments) {
		...
		if (programArguments.length == 5) {
			...
			intermediateResultsPath = programArguments[2];
			...
			System.err.println("Usage: KMeans <points path> <centers path> " +
					"<intermediate results path> <result path> <num iterations>");
			return false;
		}
